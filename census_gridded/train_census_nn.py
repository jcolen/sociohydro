...
if __name__ == '__main__':
    ...
    train_datasets = [CensusDataset(county).training() for county in train_counties]
    val_datasets = [CensusDataset(county).validate() for county in args.val_county]
    
    # Include first decade of Census data in from Figure 3 counties
    for county in args.val_county:
        train_datasets.append(torch.utils.data.Subset(
            CensusDataset(county).training(), 
