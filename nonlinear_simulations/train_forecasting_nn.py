...
if __name__ == '__main__':
    ...
    info = pd.read_csv('/home/jcolen/data/sociohydro/2024-03-26_MCPhaseDiagram/data/dynamic_keys.csv')
    ...
    if args.dynamic_type in ['segregated', 'integrated']:
        tmax = 100 # Cutoff before simulations reach steady state
    else:
        tmax = 500 # Migration phases exhibit dynamics at all times
    print(f'Using tmax={tmax} for phase {args.dynamic_type}')

    train_datasets = []
    for folder in args.train_folder:
        train_datasets.append(SimulationDataset(folder, tmax=tmax))
    train_dataset = torch.utils.data.ConcatDataset(train_datasets)
        
    val_datasets = []
    for folder in args.val_folder:
        val_datasets.append(SimulationDataset(folder, tmax=tmax))
