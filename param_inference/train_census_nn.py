...
if __name__ == '__main__':
    ...
    optimizer = torch.optim.Adam([{'params': model.coefs, 'lr': args.coef_lr},
                                  {'params': model.local_network.parameters()}],
                                 lr=args.base_lr)
    scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer,
                                                       gamma=args.scheduler_step)

    # Build train and val datasets
    dataset = CensusDataset(path=os.path.join(args.datafolder, args.county), 
                            grid=args.grid,
                            remove_extra=True,
                            preload=True,
                            region=args.region,
    ...
    time_code = dt.strftime('%Y%m%d_%H%M%S')
    savedir = os.path.join(args.savefolder,
                           f'{args.county}_{"Grid" if args.grid else "noGrid"}_{model.__class__.__name__}_{time_code}')
